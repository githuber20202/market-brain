...
def should_run(mode: str, now: datetime, calendar_path: Path) -> bool:
...
def main() -> None:
    ...
    due = should_run(args.mode, now, ROOT / "data" / "market_calendar.csv")
    value = "true" if due else "false"
    output = os.getenv("GITHUB_OUTPUT")
    if output:
        with Path(output).open("a", encoding="utf-8") as handle:
            handle.write(f"due={value}\n")
    print(f"BATCH_GATE mode={args.mode} due={value} et={now.astimezone(EASTERN).isoformat()}")
