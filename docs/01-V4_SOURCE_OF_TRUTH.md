...
בנוסף נשמר `high_52w` כשיא של 252 ימי המסחר המלאים הקודמים. קרבה לשיא היא Context בלבד: `NORMAL`, `NEAR_HIGH`, `FRESH_BREAKOUT`, `EXTENDED_BREAKOUT`, `BREAKOUT` או `MISSING`. פריצה עד `0.5x ATR14` מעל השיא מסומנת `FRESH_BREAKOUT`; מעליה `EXTENDED_BREAKOUT`. הסיווג אינו Hard Gate ואינו הופך Premarket ל־BUY. `ATH = REJECT` אינו כלל במערכת; כל מועמד עדיין חייב לעבור לאחר הפתיחה Opening Range, VWAP, Retest, Remaining ATR, Spread, Risk/Reward ו־No-Chase.
